...
export default function EventCrudActions({
  eventId,
  editHref,
  compact = false,
  onDeleted
  ...
  async function handleDuplicate() {
    ...
    try {
      ...
      if (data.event?.id) {
        router.push(`/eventos/gerenciar/${data.event.id}`);
        return;
      }
      setMessage("Evento duplicado com sucesso.");
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Erro ao duplicar evento.");
    } finally {
      setBusyAction(null);
    }
  }
